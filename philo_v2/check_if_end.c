...
int	is_dead(struct Philosopher *philo)
{
	...
	if ((time_now_ms(philo->diner_infos->program_start)
			- philo->last_eaten >= philo->diner_infos->time_to_die)
		&& (philo->state != EATING))
		return (pthread_mutex_unlock(&philo->diner_infos->eating_change_mutex),
			1);
